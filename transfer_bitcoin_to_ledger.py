...
# Setup logger
log_file = "/home/morgan/src/crypto_bot/logs/withdraw_log.txt"
...
def withdraw_to_ledger():
    try:
        ...
        btc_balance = float(balance['result'].get('XXBT', 0))  # Kraken stores BTC as 'XXBT'
        ...
        logger.info("Attempting to withdraw %.8f BTC (%.2f)to Ledger", withdraw_amount, withdraw_amount_nok)
        # Send Telegram Message
        message1 = (
                "Kraken BTC Balance: {:.8f} BTC ({:.2f} NOK)\n"
                "Leaving {:.8f} BTC ({:.2f} NOK) for kraken fees\n"
                "Attempting to withdraw {:.8f} BTC ({:.2f} NOK) to Ledger."
            ).format(btc_balance, btc_balance_nok, 
                    WITHDRAWAL_FEE, withdrawal_fee_nok, 
                    withdraw_amount, withdraw_amount_nok)

        # Withdraw request
        withdrawal = kraken.query_private('Withdraw', {
            'asset': 'XBT',
        ...
        # Send Telegram Message
        response = requests.post(url, data={"chat_id": CHAT_ID, "text": f"{message1}\n{message2}"})
    except Exception as e:
